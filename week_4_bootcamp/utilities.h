 ...
 * @param num_records count of readings
...
/**
 * @brief Checks that there was data a given record
 *
 * @param data the specific reading
 * @return int Return 0 if there are no errors, 1 if you find an error.
 */
int record_checker(reading data) 
{
    if (data.date == NULL || data.date[2] != data.date[6] || data.date[6] != '-') {
        return 1;
    } else if (data.bloodIron == 0.0){
        return 1;
    }
...
{
    for (numReadings -= 1; numReadings >= 0; numReadings--) {
        if (record_checker(dataArray[numReadings]) == 1){
            return 1;
        }
    }
 ...
 */
float find_mean(reading* dataArray, int num_readings)
{
    float sum = 0;
...
float find_highest(reading* dataArray, int numReadings)
{
    ...
    for (int i = 0; i < numReadings; i++){
        if (dataArray[i].bloodIron > max) max = dataArray[i].bloodIron;
    }
...
float find_lowest(reading* dataArray, int numReadings)
{
    float min = 100.0;
    for (int i = 0; i < numReadings; i++){
        if (dataArray[i].bloodIron < min) min = dataArray[i].bloodIron;
    }
...
void getMonth(const char *input, const char *delimiter,
                    char *month)
{
    // Create a copy of the input string as strtok modifies the string
    char *inputCopy = strdup(input);
 ...
 * @param numReadings The number of readings in the array
 */
void monthly_iron(reading* dataArray, int numReadings, char *checkMonth)
{   
    char tempMonth[4];
    for (int i = 0; i < numReadings; i++) {
        getMonth(dataArray[i].date, "-", tempMonth);
        if (strcmp(tempMonth, checkMonth) == 0) {
